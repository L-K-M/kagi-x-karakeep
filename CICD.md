...
Kagi × Karakeep is a Manifest V3 browser extension (Firefox/Gecko, via `browser_specific_settings`) that shows Karakeep bookmark results beside Kagi search results. This repository uses GitHub Actions to validate the extension on every change and to package a downloadable build whenever a version tag is pushed.
...
2. **Verifies the tag matches `manifest.json`.** The packaged add-on takes its version from `manifest.json`; the tag only names the Release. If they disagree the build fails, so a Release named `v1.5.0` can never ship a `0.1.3` extension.
3. Packages the extension:
   ...
   The artifact is version-stamped (e.g. `kagi_karakeep-1.2.3.zip`) and written to `web-ext-artifacts/`.
...
An **unsigned** package is suitable for self-distribution or temporary installation. To load it in Firefox, open `about:debugging` → **This Firefox** → **Load Temporary Add-on…** and select the `.zip`. A **signed** `.xpi` can be installed permanently.
